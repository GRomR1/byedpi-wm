...
function generateStrategies() {
    ...
    if (!validateDesyncParameters()) {
        logMessage('ОШИБКА', 'Необходимо выбрать хотя бы один десинхронизатор!');
        return;
    }

    if (!validateModifiers()) {
        logMessage('ОШИБКА', 'Необходимо выбрать хотя бы один флаг!');
        return;
    }
	
    logMessage('ИНФО', 'Начали генерацию...');	
    ...
    logMessage('ИНФО', `Сгенерировано ${strategies.length} стратегий.`);
    logMessage('ИНФО', `Уникальных стратегий: ${uniqueStrategies.length}`);
}
